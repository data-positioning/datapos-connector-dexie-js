...
const {
  ...
  vendorHomeURL: Eo,
  version: Ko
}, Co = "0.2.245", Io = "Encountered invalid container identifier", Ft = "Encountered invalid folder path", So = "Encountered invalid object path";
class zo {
  abortController;
  config;
  engineUtilities;
  ...
  async createObject(R) {
    ...
    S.version(T.verno).stores(z), S.version(T.verno + 1).stores({ [P]: R.structure || "" }), this.containers[B] = await S.open();
  }
  // Drop object
  async dropObject(R) {
    const { containerId: B, nodeId: P } = this.establishObjectIdentifiers(R.path), T = await this.establishContainer(B);
