...
function uo() {
  return br || (br = 1, (function(ye, R) {
    ((B, P) => {
      ...
      var S = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : oo, z = Object.keys, F = Array.isArray;
      function X(e, t) {
...
const {
  ...
  vendorHomeURL: Eo,
  version: Ko
}, Co = "0.2.244", Io = "Encountered invalid container identifier", Ft = "Encountered invalid folder path", So = "Encountered invalid object path";
class zo {
  abortController;
  config;
  engineUtilities;
  ...
  async createObject(R) {
    ...
    if (console.log(22), T.tables.some((F) => F.name === P)) throw new Error(`Duplicate table '${P}'.`);
    console.log(3), T.close();
    const S = new Oe(T.name);
    if (S.on("blocked", () => !1), console.log(44), T.tables.length === 0) {
    ...
    console.log(77), S.version(T.verno).stores(z), S.version(T.verno + 1).stores({ [P]: R.structure || "" }), this.containers[B] = await S.open(), console.log(88);
  }
  // Drop object
  async dropObject(R) {
    const { containerId: B, nodeId: P } = this.establishObjectIdentifiers(R.path), T = await this.establishContainer(B);
  ...
  async findObject(R) {
    ...
    return console.log("connector.findObject", P), { folderPath: R.containerId, object: void 0 };
  }
  // Get record
  async getRecord(R) {
  ...
  async retrieveRecords(R, B, P) {
    const { containerId: T, nodeId: S } = this.establishObjectIdentifiers(R.path), F = await (await this.establishContainer(T)).table(S).toArray();
