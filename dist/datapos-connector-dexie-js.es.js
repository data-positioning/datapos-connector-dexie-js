...
const {
  ...
  vendorHomeURL: Eo,
  version: Ko
}, Co = "0.2.237", Io = "Encountered invalid container identifier", Ft = "Encountered invalid folder path", So = "Encountered invalid object path";
class zo {
  abortController;
  config;
  engineUtilities;
  ...
  // Find object
  async findObject(R) {
    if (R.containerId == null) throw new Error(`${Io} '${R.containerId}'.`);
    const P = (await this.establishContainer(R.containerId)).tables.find((T) => T.name === R.nodeId);
    return console.log("findObject", P), { folderPath: R.containerId, object: P };
  }
  // Get record
  async getRecord(R) {
